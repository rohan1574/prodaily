...
const tasks: Task[] = [
  ...
  {
    name: 'Course Watching',
    frequency: 'Daily',
    icon: require('../../assets/images/Walking.png'),
  },
  {
    name: 'Organizing Home',
    frequency: 'Daily',
    icon: require('../../assets/images/Walking.png'),
  },
  {
...
const TaskItem: React.FC<{
  ...
  onFavorite: () => void;
  isFavorite: boolean;
}> = ({task, onEdit, isSelected, onFavorite, isFavorite}) => {
  ...
  return (
    <View>
      ...
      {isSelected && (
        <View
          style={tw`p-4 bg-blue-100 border-t border-blue-300 rounded-lg m-2`}>
          <Text style={tw`text-lg font-bold mb-2`}>{task.name}</Text>

          <View style={tw`mb-2`}>
            <Text style={tw`text-gray-700`}>Add Specific For:</Text>
            <TextInput
              style={tw`border p-1 rounded w-16 text-center`}
              keyboardType="numeric"
              value={specificFor}
              onChangeText={setSpecificFor}
            />
            <View style={tw`flex-row mt-1`}>
              <TouchableOpacity style={tw`bg-blue-500 px-2 py-1 rounded mx-1`}>
                <Text style={tw`text-white`}>Days</Text>
              </TouchableOpacity>
              <TouchableOpacity style={tw`bg-gray-300 px-2 py-1 rounded mx-1`}>
                <Text>Weeks</Text>
              </TouchableOpacity>
              <TouchableOpacity style={tw`bg-gray-300 px-2 py-1 rounded mx-1`}>
                <Text>Months</Text>
          ...
          <View style={tw`mb-2`}>
            ...
            <TextInput
              style={tw`border p-1 rounded w-16 text-center ml-2`}
              keyboardType="numeric"
            />
          ...
          <View style={tw`mb-2`}>
            ...
            <View style={tw`flex-row mt-1`}>
              ...
              <TouchableOpacity style={tw`bg-blue-500 px-2 py-1 rounded mx-1`}>
                <Text style={tw`text-white`}>Month</Text>
              </TouchableOpacity>
              <TouchableOpacity style={tw`bg-gray-300 px-2 py-1 rounded mx-1`}>
                <Text>Year</Text>
              </TouchableOpacity>
      ...
      <ScrollView>
        {tasks.map((task, index) => (
          <TaskItem
            ...
            task={task}
            onEdit={() =>
              setSelectedTask(selectedTask === task.name ? null : task.name)
            }
