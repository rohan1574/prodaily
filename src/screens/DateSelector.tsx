...
const DateSelector: React.FC<DateSelectorProps> = ({
  ...
  const handleDayScroll = (e: NativeSyntheticEvent<NativeScrollEvent>) => {
    const index = Math.round(
      (e.nativeEvent.contentOffset.y + ITEM_HEIGHT / 2) / ITEM_HEIGHT,
    );
      ...
      <View
        ...
        <View style={tw`bg-white rounded-xl p-4 w-11/12`}>
          ...
          <View style={tw`flex-row `}>
            {/* left - Added Date List */}
            <View style={tw`w-1/2 pl-2 w-1/2 pr-2 border-r border-gray-200`}>
              <ScrollView style={{maxHeight: 150}}>
                {addedDates.map((item, index) => (
                  <View
                    ...
                    <Text
                      style={tw`text-gray-800`}>{`${item.month} ${item.date}`}</Text>
              ...
              <TouchableOpacity
                ...
                style={tw`bg-green-500 w-20 py-2 rounded-xl mt-3`}>
                <Text style={tw`text-white text-center`}>Save</Text>
              </TouchableOpacity>

              <View style={tw`flex-row items-center mt-4`}>
                <View
                  style={tw`w-4 h-4 rounded-full border-2 border-blue-500 mr-2 justify-center items-center`}>
