...
const DayPicker: React.FC<{onCancel: () => void}> = ({onCancel}) => {
  ...
  return (
    <View style={tw`bg-gray-200 p-4 rounded-lg w-80 mx-auto`}>
      ...
      <FlatList
        ...
        renderItem={({item}) => (
          <TouchableOpacity
            style={tw`w-10 h-10 m-1 flex items-center justify-center rounded-full ${
              selectedDays.includes(item) ? 'bg-blue-500' : 'bg-gray-300'
            }`}
            onPress={() => toggleDaySelection(item)}>
            <Text
              style={tw`${
                selectedDays.includes(item) ? 'text-white' : 'text-black'
              }`}>
              {item[0]}
            </Text>
          </TouchableOpacity>
